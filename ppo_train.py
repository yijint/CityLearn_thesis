# train PPO in CityLearn on all 2022 data 
...
# logging
log_dir = f"./sb3_logs/ppo/{n_buildings}_buildings" # Directory for Monitor logs and TensorBoard logs
...
ppo_model = ppo_model.learn(
    total_timesteps = schema['simulation_end_time_step'] * N_EPISODES,
    log_interval=1
)
print(f"Total time taken to train PPO for {schema['simulation_end_time_step'] * N_EPISODES} steps: {(time.time()-start_time) / 60:.2f} minutes")

# save model
model_save_path = os.path.join(log_dir, f"ppo_model_{n_buildings}_buildings")
...
while not (terminated or truncated):
    ...
    ppo_actions_list.append(actions)
...
# save evaluation
results_dir = f'results/results_ppo_2022_centralized/{n_buildings}_buildings'
...
plt.imshow(np.stack(ppo_actions_list).T, aspect='auto', interpolation='none', cmap='coolwarm', vmin=-1, vmax=1)
...
plt.tight_layout()
plt.savefig(os.path.join(results_dir, 'ppo_actions_heatmap.png'))
